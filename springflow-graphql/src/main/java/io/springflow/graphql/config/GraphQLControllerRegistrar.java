...
public class GraphQLControllerRegistrar implements BeanFactoryPostProcessor {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        try {
            ...
            for (Class<?> entityClass : entities) {
                ...
                // Skip if custom service exists (can't wire custom services with generic GraphQL controllers)
                String serviceBeanName = getServiceBeanName(metadata);
                if (registry.containsBeanDefinition(serviceBeanName) && isCustomService(registry, serviceBeanName)) {
                    log.info("Skipping GraphQL controller for {} - custom service {} exists",
                            metadata.entityName(), serviceBeanName);
                    continue;
                }
    ...
    /**
     * Gets the service bean name for an entity.
     */
    private String getServiceBeanName(EntityMetadata metadata) {
        return Character.toLowerCase(metadata.entityName().charAt(0)) + metadata.entityName().substring(1) + "Service";
    }

    /**
     * Checks if a service bean is a custom implementation (not generated by SpringFlow).
     */
    private boolean isCustomService(BeanDefinitionRegistry registry, String serviceBeanName) {
        try {
            var beanDef = registry.getBeanDefinition(serviceBeanName);
            // Generated services use SpringFlowServiceFactoryBean
            String beanClassName = beanDef.getBeanClassName();
            return beanClassName != null && !beanClassName.contains("SpringFlowServiceFactoryBean");
        } catch (Exception e) {
            return false;
        }
    }
